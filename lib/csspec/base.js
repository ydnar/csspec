  ...
  CSSpec.$.extend(CSSpec, {
    ...
    run: function() {
      ...
      if(params.start <= params.end) {
        ...
        var location = ("" + CSSpec.global.location).replace(/(\?.+)?$/, search);
        CSSpec.global.location = location;
      }
    },
    
    enqueue: function() {
      CSSpec.global.setTimeout(function() { CSSpec.run(); }, 0);
    },
    
    parseLocation: function() {
      var params = {};
      var search = "" + CSSpec.global.location.search;
